...
$port = getenv('BENCHMARK_PORT') ?: '8083';
...
$exit = match ($action) {
   'start' => (function () use ($bootglyDir, $port): int {
      ...
      exec("php {$bootglyDir}/bootgly project Benchmark/TCP_Server_CLI stop > /dev/null 2>&1");
      usleep(500_000);
      ...
      return ServerCapture::run("{$env}php {$bootglyDir}/bootgly project Benchmark/TCP_Server_CLI start");
   })(),
