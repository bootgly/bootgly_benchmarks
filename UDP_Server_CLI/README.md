...
| `connections` | Number of concurrent UDP sockets |
| `client-workers` | Number of client worker processes |

```bash
# Vary server workers and connections (cartesian product)
./bootgly test benchmark UDP_Server_CLI --vary=server-workers:4,connections:256
```
...
```
bootgly/workdata/tests/benchmarks/UDP_Server_CLI/<timestamp>_bench.marks
```
