...
) extends Runner
{
   ...
   private function startServer (Opponent $Opponent, int $workers): void
   {
      ...
      $this->ServerArtifacts = RunArtifacts::create('udp-raw-server');
   ...
   {
      $script = $Opponent->script;
      $Artifacts = RunArtifacts::create('udp-raw-server-stop');
   ...
   private function warmup (): void
   {
      ...
      $Artifacts = RunArtifacts::create('udp-raw-warmup');
      $JSON = json_encode([
         'message' => "PING\n",
      ], JSON_THROW_ON_ERROR);
   ...
   private function command (Load $Load, int $connections = 0, int $clientWorkers = 0): Result
   {
      ...
      $Artifacts = RunArtifacts::create('udp-raw-load');
